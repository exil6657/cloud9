...
struct RectCommand { Vec2 minimum; Vec2 maximum; Color color; bool filled{true}; float rounding{0.0F}; };
struct BoxCommand { Vec3 minimum; Vec3 maximum; Color color; bool filled{false}; };
using RenderCommand = std::variant<LineCommand, TextCommand, RectCommand, BoxCommand>;
...
public:
    ...
    void rect(RectCommand command) { commands_.emplace_back(std::move(command)); }
